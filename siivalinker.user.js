...
(function() {
  ...
  const suggestionUrl = new URL('https://www.siivagunner.wiki/w/rest.php/v1/search/title');
    suggestionUrl.searchParams.set('limit', '10');

  const wikiUrlBase = 'https://www.siivagunner.wiki/wiki/$1';
  ...
  const processSuggestions = function(res) {
    if (res.status === 200) {
      ...
      let suggestions = resObj.pages;
      dbg('suggestions', suggestions);

      if (suggestions.length > 0) {
        let sug = suggestions[0];
        dbg('sug', sug);
        let valueEnc = sug.key;
